  ...
  <BaseModal :visible="visible" @close="$emit('close')" title="AWS PostgreSQL Migration">
    ...
    <div class="px-4 pb-4 flex flex-col md:flex-row gap-4 md:items-center">
      <p class="text-base text-gray-300 leading-relaxed flex-1">
        PostgreSQL migration to Amazon RDS with Terraform provisioning VPC, security groups, SSM bastion access, and Secrets Manager credentials. Uses pg_dump/pg_restore for simple, reproducible migrations. Multi-AZ ready with enhanced monitoring and CloudWatch metrics.
      </p>
      <a
