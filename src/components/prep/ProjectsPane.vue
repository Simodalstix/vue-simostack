<template>
  <div class="flex flex-col h-full overflow-hidden font-mono">
    ...
    <!-- Project tab bar -->
    <div class="shrink-0 flex border-b border-slate-700 bg-slate-800/20">
      <button
    ...
    <!-- Main content -->
    <main class="flex-1 overflow-y-auto p-5 bg-slate-900">
      <template v-if="project">

        <!-- Header -->
        <div class="mb-5 flex items-start gap-6">
          <div class="shrink-0">
            <h2 class="text-orange-300 font-semibold text-[13px] uppercase tracking-widest mb-2">
              {{ project.label }}
            </h2>
          ...
          <template v-for="(sec, i) in project.sections" :key="i">
            <div :class="sec.span === 2 ? 'col-span-2' : 'col-span-1'">
              ...
              <template v-if="sec.type === 'prose'">
                <div class="border border-slate-700/50 rounded-lg bg-slate-800/25 px-4 py-3">
                  <div v-if="sec.heading" class="text-[10px] uppercase tracking-widest text-orange-400 font-semibold mb-2">
                    {{ sec.heading }}
                  </div>
                  <p class="text-slate-300 text-[12px] leading-relaxed">{{ sec.body }}</p>
                </div>
              </template>

              <!-- ── kv grid ─────────────────────────────────────── -->
              <template v-else-if="sec.type === 'kv'">
                <div class="border border-slate-700/50 rounded-lg bg-slate-800/25 overflow-hidden h-full">
                  <div v-if="sec.heading" class="px-3 py-2 border-b border-slate-700/40">
                    <span class="text-[10px] uppercase tracking-widest text-orange-400 font-semibold">{{ sec.heading }}</span>
                  </div>
                  <div class="px-3 py-2.5 grid gap-y-1.5 gap-x-3 text-[11px]" style="grid-template-columns: 8.5rem 1fr">
                    <template v-for="row in sec.rows" :key="row.key">
                      ...
                      <span class="text-slate-400 leading-snug">{{ row.value }}</span>
                    </template>
                  </div>
                </div>
              </template>
              ...
              <template v-else-if="sec.type === 'code'">
                <div class="border border-slate-700/50 rounded-lg bg-slate-800/25 overflow-hidden h-full">
                  ...
                  <div v-if="sec.label" class="px-3 pt-2 text-[10px] text-slate-500 italic">{{ sec.label }}</div>
                    ...
                    <template v-for="(step, si) in sec.steps" :key="si">
                      <div class="flex flex-col items-center min-w-[80px] max-w-[140px]">
                        <div class="w-full px-2 py-2 border border-sky-700/60 bg-sky-950/40 rounded text-center">
                          ...
                          <div v-if="step.sub" class="text-slate-500 text-[10px] mt-0.5 leading-tight">{{ step.sub }}</div>
                        </div>
                      </div>
                      <span v-if="si < sec.steps.length - 1" class="text-slate-600 text-[14px] shrink-0">→</span>
                    </template>
                  </div>
                </div>
              </template>
                      ...
                      <thead>
                        <tr class="border-b border-slate-700/60">
                          <th
                            ...
                            :class="col ? 'text-sky-300' : 'text-slate-500'"
                          >{{ col }}</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr v-for="(row, ri) in sec.rows" :key="ri" class="border-b border-slate-700/20">
                          <td class="py-1.5 pr-4 text-slate-500 font-medium">{{ row.label }}</td>
                          <td v-for="(val, vi) in row.vals" :key="vi" class="py-1.5 pr-4 text-slate-300">{{ val }}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
              ...
              <template v-else-if="sec.type === 'probes'">
                <div class="border border-slate-700/50 rounded-lg bg-slate-800/25 overflow-hidden">
                  <div class="px-3 py-2 border-b border-slate-700/40">
                    <span class="text-[10px] uppercase tracking-widest text-orange-400 font-semibold">
                  ...
                  <div class="px-3 py-3 space-y-4">
                    <div v-for="(probe, pi) in sec.items" :key="pi">
                      ...
                      <div class="border-l-2 border-slate-700 pl-3 text-slate-400 text-[11px] leading-relaxed">{{ probe.a }}</div>
                    </div>
                  </div>
                </div>
              </template>
                ...
                <div class="border border-slate-700/50 rounded-lg bg-slate-800/25 overflow-hidden h-full">
                  ...
                  <div class="px-3 py-3 space-y-3">
                    <div
                      ...
                      class="pl-3 border-l-2 border-amber-500/40 text-slate-300 italic text-[11px] leading-relaxed"
                    >{{ q }}</div>
                  </div>
                </div>
              </template>
              ...
              <template v-else-if="sec.type === 'stack'">
                <div class="flex flex-col gap-3 h-full">
                  <div v-for="(child, ci) in sec.items" :key="ci" class="flex-1">
                    <div v-if="child.type === 'kv'" class="border border-slate-700/50 rounded-lg bg-slate-800/25 overflow-hidden h-full">
                      <div v-if="child.heading" class="px-3 py-2 border-b border-slate-700/40">
                        <span class="text-[10px] uppercase tracking-widest text-orange-400 font-semibold">{{ child.heading }}</span>
                      </div>
                      <div class="px-3 py-2.5 grid gap-y-1.5 gap-x-3 text-[11px]" style="grid-template-columns: 8.5rem 1fr">
                    ...
                    <div v-else-if="child.type === 'prose'" class="border border-slate-700/50 rounded-lg bg-slate-800/25 px-4 py-3 h-full">
                      <div v-if="child.heading" class="text-[10px] uppercase tracking-widest text-orange-400 font-semibold mb-2">{{ child.heading }}</div>
                      <p class="text-slate-300 text-[12px] leading-relaxed">{{ child.body }}</p>
                    </div>
                    <div v-else-if="child.type === 'bullets'" class="border border-slate-700/50 rounded-lg bg-slate-800/25 overflow-hidden h-full">
                      <div v-if="child.heading" class="px-3 py-2 border-b border-slate-700/40">
                        <span class="text-[10px] uppercase tracking-widest text-orange-400 font-semibold">{{ child.heading }}</span>
                      </div>
                      <div class="px-3 py-2.5 space-y-1.5">
