<template>
  <div class="flex h-full text-[12px]">
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
      ...
      <div class="border border-slate-700/50 rounded-lg bg-slate-800/25 overflow-hidden">
        <div class="px-3 py-2 border-b border-slate-700/40">
          <span class="text-[10px] uppercase tracking-widest text-orange-400 font-semibold">Interview Framing</span>
        </div>
        <div class="px-3 py-2.5 space-y-1.5 text-[11px]">
          <div class="flex gap-1.5"><span class="text-slate-500 shrink-0">•</span><span class="text-slate-300">Python for repeatable operational tasks — not one-offs, not interactive tools.</span></div>
          <div class="flex gap-1.5"><span class="text-slate-500 shrink-0">•</span><span class="text-slate-300">Design around clear inputs, predictable output, and correct exit codes.</span></div>
          <div class="flex gap-1.5"><span class="text-slate-500 shrink-0">•</span><span class="text-slate-300">Use boto3 with IAM roles — never static keys, never hardcoded credentials.</span></div>
          <div class="flex gap-1.5"><span class="text-slate-500 shrink-0">•</span><span class="text-slate-300">Keep scripts simple, self-contained, and testable in isolation.</span></div>
        </div>
      </div>

      <!-- Python pattern cards -->
      <div v-for="script in pythonScripts" :key="script.id" class="border border-slate-700/50 rounded-lg bg-slate-800/25 overflow-hidden">
      ...
      <div class="border border-slate-700/50 rounded-lg bg-slate-800/25 overflow-hidden">
        ...
        <div class="px-3 py-2.5 space-y-2 text-[11px]">
          ...
          <div class="pl-3 border-l-2 border-amber-500/40 text-slate-300 italic leading-relaxed">"boto3 picks up the instance profile automatically on EC2 — no keys in code, ever."</div>
          <div class="pl-3 border-l-2 border-amber-500/40 text-slate-300 italic leading-relaxed">"Exit codes matter in scripts — 0 means healthy, 1 means action needed. Cron and pipelines depend on that contract."</div>
        </div>
      </div>
