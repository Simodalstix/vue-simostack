<template>
  <div style="height: 100%; overflow: auto; padding: 16px 20px; background: #0f172a; font-family: ui-monospace, monospace;">
    ...
    <!-- Standalone items below the matrix -->
    <div style="margin-top: 16px; margin-left: 88px; display: flex; gap: 12px; flex-wrap: wrap;">
      <div
        v-for="item in standaloneItems"
        :key="item.label"
        style="padding: 10px 14px; border: 1px dashed #334155; border-radius: 4px; min-width: 280px; flex: 1;"
      >
        <div style="font-size: 11px; font-weight: 700; color: #64748b; letter-spacing: 0.09em; text-transform: uppercase; margin-bottom: 8px;">
          {{ item.label }}
        </div>
        <div
          v-for="(anchor, ai) in item.anchors"
          :key="ai"
          style="font-size: 12px; color: #64748b; line-height: 1.45; margin-bottom: 4px;"
        >
