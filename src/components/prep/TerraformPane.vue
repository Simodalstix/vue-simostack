<template>
  <div class="h-full overflow-y-auto p-4 font-mono text-[12px]">
    ...
    <div class="text-[10px] uppercase tracking-widest text-ob-sand font-semibold pb-1.5 border-b border-ob-text/14 mb-3">
      Terraform / IaC
    </div>
    ...
    <div class="grid grid-cols-3 gap-3 mb-3">
      ...
      <!-- S3 Bucket Example -->
      <div class="row-span-2 border border-ob-text/16 rounded-lg bg-ob-surface2/25 overflow-hidden">
        <div class="px-3 py-2 border-b border-ob-text/14">
    ...
    <div class="grid grid-cols-3 gap-3">
      ...
      <div class="border border-ob-text/16 rounded-lg bg-ob-surface2/25 overflow-hidden">
        ...
        <div class="px-3 py-2.5 space-y-2 text-[11px]">
          <div class="flex gap-1.5"><span class="text-ob-dim shrink-0">•</span><span class="text-ob-text">Declarative means TF owns the diff — HCL describes end state, TF builds a <span class="text-sky-300">dependency graph (DAG)</span> to reach it. Idempotent: same config → same outcome.</span></div>
          <div class="flex gap-1.5"><span class="text-ob-dim shrink-0">•</span><span class="text-ob-text"><span class="text-sky-300">plan -out</span> in CI freezes the diff so apply executes exactly what was reviewed. Without it there's a race between plan and apply.</span></div>
          <div class="flex gap-1.5"><span class="text-ob-dim shrink-0">•</span><span class="text-ob-text">S3 stores state, DynamoDB provides a <span class="text-sky-300">single-writer lock</span>. Two concurrent applies without it → state corruption, no clean rollback.</span></div>
          <div class="flex gap-1.5"><span class="text-ob-dim shrink-0">•</span><span class="text-ob-text">State drift: plan on untouched code still shows changes if someone edited in console. Fix: <span class="text-sky-300">terraform import</span> to adopt it, or realign the real resource.</span></div>
          <div class="flex gap-1.5"><span class="text-ob-dim shrink-0">•</span><span class="text-ob-text"><span class="text-sky-300">for_each</span> over <span class="text-sky-300">count</span> for lists — count uses a numeric index so renaming an item destroys everything after it. for_each uses a stable string key.</span></div>
          <div class="flex gap-1.5"><span class="text-ob-dim shrink-0">•</span><span class="text-ob-text">State holds secrets in <span class="text-sky-300">plaintext</span> — restrict the S3 bucket, enable SSE, never commit tfstate to git.</span></div>
          <div class="flex gap-1.5"><span class="text-ob-dim shrink-0">•</span><span class="text-ob-text">Don't share resource ownership with CDK — two tools managing the same resource means last apply wins and breaks the other.</span></div>
        </div>
      </div>
      ...
      <div class="border border-ob-text/16 rounded-lg bg-ob-surface2/25 overflow-hidden">
        ...
        <div class="px-3 py-2.5 space-y-2 text-[11px]">
          <div class="pl-3 border-l-2 border-amber-500/40 text-ob-text italic leading-relaxed">"plan -out saves the exact plan to a file so CI applies precisely what was reviewed. Without it there's a race condition between plan and apply."</div>
          <div class="pl-3 border-l-2 border-amber-500/40 text-ob-text italic leading-relaxed">"plan on a clean codebase still shows changes if someone touched the resource manually — that's drift, and it's how you catch console cowboys."</div>
          <div class="pl-3 border-l-2 border-amber-500/40 text-ob-text italic leading-relaxed">"state rm removes a resource from state without destroying it — useful when you want Terraform to stop managing something without touching the real resource."</div>
        </div>
      </div>
