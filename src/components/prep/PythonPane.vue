<template>
  <div class="flex flex-col h-full overflow-hidden font-mono">
    ...
    <!-- LEFT column -->
    <div class="flex-[2] overflow-y-auto p-5 border-r border-ob-text/18 space-y-6">
      ...
      <!-- When Python wins -->
      <div>
        <div class="text-[11px] text-ob-text leading-relaxed mb-3">
        ...
        <div class="text-[10px] uppercase tracking-widest text-ob-dim mb-2">When Python wins over bash</div>
        <ul class="space-y-1">
          <li v-for="point in whenPython" :key="point" class="flex gap-2 text-[11px] text-ob-text">
            <span class="text-ob-sand/50 shrink-0">›</span>{{ point }}
          </li>
        </ul>
      </div>

      <!-- Left scripts -->
      <div v-for="script in leftScripts" :key="script.id" class="border border-ob-text/16 rounded-lg bg-ob-surface2/25 overflow-hidden">
        <div class="px-3 py-2 border-b border-ob-text/14 flex items-center gap-2 flex-wrap">
          ...
          <div class="flex gap-1 ml-auto flex-wrap justify-end">
            <span v-for="tag in script.tags" :key="tag"
                  class="px-1.5 py-0.5 text-[9px] bg-ob-surface/80 text-ob-text rounded font-mono">{{ tag }}</span>
            <span class="px-1.5 py-0.5 text-[9px] bg-emerald-900/50 text-emerald-400 rounded">stdlib</span>
          </div>
        </div>
        <div class="px-3 py-2 border-b border-ob-text/14 grid grid-cols-[3fr_2fr] gap-4 text-[11px]">
          <div>
    ...
    <div class="flex-[3] overflow-y-auto p-5 space-y-6">
      ...
      <div v-for="script in rightScripts" :key="script.id" class="border border-ob-text/16 rounded-lg bg-ob-surface2/25 overflow-hidden">
        <div class="px-3 py-2 border-b border-ob-text/14 flex items-center gap-2 flex-wrap">
          <span class="text-[10px] uppercase tracking-widest text-ob-sand font-semibold">{{ script.heading }}</span>
          <div class="flex gap-1 ml-auto flex-wrap justify-end">
            <span v-for="tag in script.tags" :key="tag"
