<template>
  <section class="rounded-[6px] border border-ob-sand/10 bg-ob-surface2 px-4 py-3.5">
    ...
    <template v-if="safety">
      <div class="mt-2 flex flex-wrap items-baseline gap-x-2 gap-y-1">
        <span class="font-mono text-[21px] leading-none text-ob-teal"
          >{{ safety.score.toFixed(1) }}/10</span
        >
        <span class="text-[11.5px] text-ob-muted2">{{ band }}</span>
      </div>
      <p class="mt-2 text-[11.5px] leading-relaxed text-ob-muted2">
      ...
      <div class="mt-3 grid grid-cols-2 gap-x-3 gap-y-1.5">
        <div v-for="basket in safetyContext.baskets" :key="basket.key" class="min-w-0">
          <p class="font-mono text-[9.5px] uppercase tracking-[0.05em] text-ob-faint">
            {{ basket.label }} · {{ Math.round(basket.weight * 100) }}%
          </p>
          <p
            class="font-mono text-[11px]"
            :class="riskClass(safety.baskets[basket.key]?.riskPercentile)"
          >
            {{ riskLabel(safety.baskets[basket.key]?.riskPercentile) }}
          </p>
        </div>
      </div>
      <details class="mt-3 border-t border-ob-sand/10 pt-2">
    ...
    <p v-else class="mt-2 text-[11.5px] italic text-ob-faint">
...
const props = defineProps({ areaId: { type: String, required: true } })
