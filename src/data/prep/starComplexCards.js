...
What stayed with me was how much the redirect URL mattered. A generic access denied message would've sent me in completely the wrong direction. Pattern recognition on one detail — knowing that URL was Duo, knowing that was wrong — that was the whole investigation.`,
    cues: [
      'SITUATION',
      'Live call, user can\'t log in — hybrid identity environment, unfamiliar error message',
      'Remoted in to observe directly rather than troubleshoot blind',
      '---',
      'ACTION',
      'The URL told me more than the error — it was a Duo MFA redirect, and Duo had been decommissioned',
      'Asked if it prompted for a code — he said no. That confirmed the auth flow itself was broken, not the credentials',
      'Checked Entra sign-in logs, then pulled Conditional Access policies — found a ghost policy still enforcing Duo',
      'Policy was scoped to a \'Duo Members\' group — group lived in on-prem AD, synced to Entra',
      'On-prem is authoritative — couldn\'t edit group membership from the cloud side',
      'RDP\'d into the DC, removed him from the Duo Members group, attempted a delta sync — it threw an error',
      'Identified that the DC and the Azure AD Connect sync agent are on separate VMs — segregated infrastructure',
      'Hit my access boundary — brought the ops manager in with the full picture: policy → group → on-prem → topology constraint',
      'Ops manager was already working on the Azure AD Connect VM — forced the sync manually',
      '---',
      'RESULT',
      ...
      '---',
      'CLOSE',
      'The error message told me nothing. The redirect URL told me everything — that\'s what set the direction of the whole investigation.',
      'I considered excluding the user from the CA policy as a quick fix — but that would\'ve left the ghost policy active for anyone else in that group. The right fix was removing the stale membership, not papering over it.',
    ],
