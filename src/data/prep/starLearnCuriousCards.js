export const starLearnCuriousCards = [
  {
    ...
    anchors: [
      ...
      { tag: '[DATA]', code: 'SELECT * FROM user_roles WHERE user_id = ?\n-- validate before touching' },
      { tag: '[DATA]', code: 'UPDATE user_roles SET role = ? WHERE user_id = ?\n-- single field, single row' },
...
Nobody told me to set up the local environment, take the SQL course, or map the call categories. I did it because I could see what the job actually required and I wasn't going to get there by waiting.`,
    cues: [
      'SITUATION',
      'First real tech role — onboarding was very short, basically one setup day and a few days of shadowing and reading',
      'Company was fully remote, so proper shadowing wasn\'t always practical',
      'By week one I was already taking live calls independently for pharmacy clients across Australia',
      '---',
      'ACTION',
      'Realised pretty quickly that waiting for formal training wasn\'t going to be enough',
      'Installed SQL Express locally, set up the application myself, and recreated common workflows to build familiarity before seeing issues live with customers',
      'Started tracking recurring support patterns — cannabis integrations, connection issues, RAM — and focused heavily on those areas first',
      'Outside work: took a SQL course and used AI heavily to understand SQL Server and the connection layer, not just memorise support steps',
      '---',
      'ACTION (SQL moment)',
      'Intermittent pharmacist role issue that normally needed manager intervention',
      'Fix was a very small SQL update',
      'Before touching it: validated exactly what the query was doing, confirmed it only affected a single field on a single record, made sure I understood the risk',
      'Once I was confident it was safe, handled it myself instead of escalating',
      '---',
      'RESULT',
      'Within the first couple of months became much more independent with the platform, escalated far fewer calls',
      'Became one of the go-to people for some recurring issue types',
      'SQL moment was important — first time I felt confident assessing technical risk myself instead of just following a process',
      '---',
      'CLOSE',
      'This was my first tech role — something I\'d been working toward for a long time',
      'Sitting around waiting for more training just wouldn\'t have felt right',
      'I was at home anyway — why not use that time to actually get better at the job I\'d wanted?',
      'That motivation is still there — it\'s not something I\'ve had to manufacture',
    ],
