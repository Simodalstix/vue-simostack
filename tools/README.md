...
| `dwelling-greenspace/` | Population-weighted greenspace access; also owns the shared 70-record target registry `dwelling-greenspace-targets.json` used by the cost pipeline too. | `source/Mesh_Block_Counts_2021.xlsx` (tracked); large geometry in untracked `.cache/` | `dwelling-greenspace-START-HERE.md` |
